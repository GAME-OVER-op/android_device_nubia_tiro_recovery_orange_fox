...

{
  ...
  echo "haptics_backend=nubia-awinic-continuous-with-persistent-input-ff-fallback"
  echo "orangefox_app_manager=enabled"
  echo "root_module_manager=orangefox-built-in-magisk-apatch-kernelsu"
  echo "post_flash_buttons=native-orangefox-style-chain-with-text"
  echo "post_flash_button_resources=verified-or-injected-into-active-orangefox-theme"
  echo "post_flash_cache_action=dalvik-only"
  echo "persistent_recovery_logs=data-fallback-no-cache-partition"
  echo "removed_incompatible_modules=nt38771_touch,si_haptic,xiaomi_touch"
